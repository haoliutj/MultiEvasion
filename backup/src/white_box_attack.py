"""
scale to torchattacks library;
CW attack and BIM attack
"""
...
def test_loop(model,data_iterator,adversary,device):
    ...
    for i,batch in enumerate(data_iterator):
        ...
        adv_x = get_adv_x_y(embed_x,y,model,adversary)
        # get label for adv_x
        adv_x = torch.Tensor(adv_x).to(device)
        embed_adv_x = embed(adv_x.long().to(device)).detach()
        adv_pred = model(embed_adv_x.to(device))
...
def main_test(opts):
    ...
    if opts['adversary'] == 'FGSM':
        ...
        # adversary = torchattacks.CW(model, c=1, kappa=0, steps=10, lr=0.01,payload_size=payload_size)
        adversary = torchattacks.CW_1(model, c=1, kappa=0, steps=100, lr=0.1,payload_size=payload_size)
