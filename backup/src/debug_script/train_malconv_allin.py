...
import numpy as np
import pandas as pd
from src.util import ExeDataset_allin, my_collate
from src.model import MalConv_allin
...
def train_loop(epochs,model,train_data,val_data,lr,model_path,device):
    ...
    for epoch in range(1,epochs+1):
        ...
        Train Loss: {train_loss:.3f} | Train Acc: {train_acc:.3f} | Val. Loss: {valid_loss:.3f} | \
         Val. Acc: {valid_acc:.3f}')
...
def main_train(opts):
    ...
    train_data_loader = DataLoader(ExeDataset_allin(list(train_label_table.index), opts['train_data_path'],
                                              list(train_label_table.ground_truth),opts['first_n_byte']), batch_size=opts['batch_size'],
                                   shuffle=True, num_workers=opts['use_cpu'],collate_fn=my_collate)
    valid_data_loader = DataLoader(ExeDataset_allin(list(val_label_table.index), opts['valid_data_path'],
                                              list(val_label_table.ground_truth),opts['first_n_byte']), batch_size=opts['batch_size'],
                                   shuffle=False, num_workers=opts['use_cpu'],collate_fn=my_collate)
...
if __name__ == '__main__':
    ...
    config_path = '../../config/config_malconv.yaml'
