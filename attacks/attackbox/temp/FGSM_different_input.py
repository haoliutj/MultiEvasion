...
class FGSM(Attack):
    ...
    def __init__(self,
        ...
        ## 继承父类Attack的参数
        super(FGSM, self).__init__(model_1, model_2, width=width,height=height)
        ## 不同方式继承父类
    ...
    def perturbation(self,
                     x_byte:list=None,
                     x_image=None,  # format from image, ndarray or tensor
        ...
        if len(index_to_perturb) > 0:
            ...
            if self.model_2:
                (adv_x_preds_1,adv_y_1),(adv_x_preds_2,adv_y_2)=self._get_print_return_results(x_byte=x_byte,
                                                                                               x_image=x_image,
                                                                                               model_byte=self.model_1,
                                                                                               model_image= self.model_2,
                                                                                               test_x_init=True)
                ...
                adv_x_preds_1, adv_y_1 = self._get_print_return_results(x_byte, self.model_1)
                pert_size = len(self.index_to_perturb)
        ...
        x_adv_byte_init,x_adv_image_init,pert_init = self.apply_manipulations(x_byte=x_byte,
                                                        x_image=x_image,
                                                        index_to_perturb=self.index_to_perturb,
                                                        pert_init_with_benign=self.pert_init_with_benign,
        ...
        (adv_x_preds_1,adv_y_1),(adv_x_preds_2,adv_y_2) = self._get_print_return_results(x_byte=x_adv_byte_init,
                                                                                         x_image=x_adv_image_init,
                                                                                         model_byte=self.model_1,
                                                                                         model_image=self.model_2,
                                                                                         test_x_init=True)
        ...
        ## add dim at axis=0
        x_adv_byte = torch.tensor(np.array(x_adv_byte_init)[np.newaxis,:],device=self.device)
        x_adv_image = torch.tensor(np.array(x_adv_image_init)[np.newaxis,:],device=self.device)
        ...
        x_adv_byte = Variable(x_adv_byte.float(), requires_grad=True).to(self.device)
        x_adv_image = Variable(x_adv_image.float(), requires_grad=True).to(self.device)

        # forward process
        preds_1, embed_x_1 = self._forward(x_adv_byte, self.model_1,model_type='byte') # embed_x: float
        ...
        if not self.model_2:
            ...
            preds_2, embed_x_2 = self._forward(x_adv_image, self.model_2)
        ...
        ## get final sign
        grad_sign_for_perturb_index,num_index_change_by_gradient = self._get_final_grad_sign(grad_sign_1=grad_sign_for_perturb_index_1,
                                                                grad_sign_2=grad_sign_for_perturb_index_2,
                                                                overshot=0.07)

        ## get final adv_x from two inputs
        x_adv_byte,x_adv_image = self._get_final_advX(x_byte=x_byte,
                                                      x_image=x_image,
                                                      gradient_sign_for_perturb_index=grad_sign_for_perturb_index,
                                                      eps=self.eps)

        ## append num_index_change to list for records
        num_index_change_in_total_group.append(len(self.index_to_perturb))
        ...
        if self.model_2:
            (adv_x_preds_1,adv_y_1),(adv_x_preds_2,adv_y_2) = \
                self._get_print_return_results(x_byte=x_adv_byte,
                                               x_image=x_adv_image,
                                               model_byte=self.model_1,
                                               model_image=self.model_2,
                                               test_x_init=False)
