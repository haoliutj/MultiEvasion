...
1.convert exe to image: binary file --> image, where the width and height of image is
based on the size of image
2. load image: use transform to load image, include transform.resize, randomCrop, etc
...
class train_imageNet:
    ...
    def load_data_1(self,
        ...
        data_loader = torch.utils.data.DataLoader(ImageDataset(list(data_label_table.index),data_path,list(data_label_table.ground_truth),
    ...
    def plot_loss_acc(self,train_loss=None,val_loss=None,train_acc=None,val_acc=None):
        ...
        plt.savefig(save_path + args.model_name+'_epoch' +str(args.epochs) + '_batchsize' + \
                           str(args.batch_size)+'_resolution'+str(args.image_resolution)+\
                           '_resize'+str(args.resize_value) + '_train_val_acc.png')
        ...
        plt.savefig(save_path + args.model_name+'_epoch' +str(args.epochs) + '_batchsize' + \
                           str(args.batch_size)+'_resolution'+str(args.image_resolution)+\
                           '_resize'+str(args.resize_value) + '_train_val_loss.png')
    ...
    def train_loop(self,
        ...
        with open(args.log_file_name,'a') as f:
            ...
            print(f'Precision: {precision}, Recall: {recall}, F1: {f1}',file=f)
            print('\n',file=f)
    ...
    def main(self,image_formal_loading=False):
        ...
        if image_formal_loading:
            ## load image with API and perform transformations
...
if __name__ == '__main__':
    ...
    args.log_file_name = ('.').join(log_file_split[:-1]) + args.model_name +'_epoch' +str(args.epochs) + '_batchsize' + \
                           str(args.batch_size)+'_resolution'+str(args.image_resolution)+'_resize'+str(args.resize_value) + '.txt'
    ...
    args.model_save_path = ('.').join(model_name_split[:-1]) + \
                           ...
                           str(args.batch_size)+'_resolution'+str(args.image_resolution)+\
                           '_resize'+str(args.resize_value)+ '.pth'

    ## training
    start = time.time()
    train_model = train_imageNet()
    train_model.main(image_formal_loading=True)
