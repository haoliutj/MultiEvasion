...
import numpy as np
import pandas as pd
from src.util import ExeDataset_cnn, params_cnn
...
def main_train(opts,nn_params):
    ...
    train_data_loader = DataLoader(ExeDataset_cnn(list(train_label_table.index), opts['train_data_path'],
                                              list(train_label_table.ground_truth),opts['first_n_byte']), batch_size=opts['batch_size'],
                                   shuffle=True, num_workers=opts['use_cpu'])
    valid_data_loader = DataLoader(ExeDataset_cnn(list(val_label_table.index), opts['valid_data_path'],
...
if __name__ == '__main__':
    ...
    config_path = '../../config/config_cnn.yaml'

    opts = yaml.load(open(config_path,'r'))
    nn_params = params_cnn(1, opts['first_n_byte'])
    main_train(opts,nn_params)
