"""
produce generic adversarial malware that evade two malware detectors with different input format,
(one is raw byte based, one is image based)
...
from src.util import ExeImageDataset, get_acc_FP, get_adv_name
from src.model import MalConv_freezeEmbed,ResNet18
import numpy as np

from attacks.attackbox.temp.FGSM_different_input import FGSM
from attacks.attackbox import PGD

from attacks.attackbox.attack_utils import get_perturb_index_and_init_x
from src.util import forward_prediction_process
from attacks.attackbox.temp.attack_base_different_input import Attack
...
class adv_evasion_attack_sync(Attack):
    ...
    """
    def __init__(self,
                 model1=None, ## raw byte based
                 model2=None, ## image based
    ...
    def get_predicted_malware(self,
        ...
        X_byte_mal,X_image_mal,y_mal = [],[],[] ## to record samples and lables that predicted as malware by one detector at least
        y_pred_1, y_pred_2 = [],[]  ## to record the predicted labels for each model
        all_y = [] ## to record orgianl labels of all input
        for i,batch in enumerate(data_iterator):
            (X_byte,X_image),y = batch
            X_byte,X_image,y = X_byte.float().to(self.device), X_image.float().to(self.device), y.long().to(self.device)
            all_y.append(y.item())

            confidence_1,_ = forward_prediction._forward(X_byte, model_1,model_type='byte')
            y_pred_1.append(np.argmax(confidence_1))
            if model_2:
                confidence_2,_ = forward_prediction._forward(X_image, model_2,model_type='image')
                y_pred_2.append(np.argmax(confidence_2))
                if confidence_1[0] > 0.5 and confidence_2[0] > 0.5:
                    continue
                else:
                    X_byte_mal.append(X_byte.cpu().detach().squeeze().numpy())
                    X_image_mal.append(X_image.cpu().detach().squeeze().numpy())
                    y_mal.append(y.item())
            else:
                if confidence_1[0] < 0.5:
                    X_byte_mal.append(X_byte.cpu().detach().squeeze().numpy())
                    y_mal.append(y.item())
        print(f"Add {len(X_byte_mal)} Malwares.")
        print(
            f'Model1 MalConv Acc: {accuracy_score(y_pred_1, all_y)}; Model2 Image Acc: {accuracy_score(y_pred_2, all_y)}')
        ...
        if verbose:
            with open(self.log_file_for_result, 'a') as f:
                ...
                print(f'{len(X_byte_mal)} malware predicted as malware at least by one detector', file=f)
                print(f'These {len(X_byte_mal)} malware will be used to produce adversarial malware.', file=f)
                print(f'Model1 MalConv Acc: {accuracy_score(y_pred_1,all_y)}; Model2 FireEye Acc: {accuracy_score(y_pred_2,all_y)}',file=f)
                print('\n',file=f)

        return (X_byte_mal,X_image_mal),y_mal
    ...
    def test_loop(self,
        ...
        for i,(X,y) in enumerate(zip(X_mal,y_mal)):
            ...
            if model_2:
                ...
                num_index_change_by_gradient_group = adversary.perturbation(x_byte=X_init,
                                                                            x_image= 0,
...
def main_test():
    ...
    transform = transforms.Compose([transforms.Grayscale(num_output_channels=1),
                                    ...
                                    transforms.ToTensor()])
    test_data_loader = DataLoader(ExeImageDataset(fp_list=list(test_label_table.index),
                                                  data_path=args.test_data_path,
                                                  image_data_path='../../data/image/',
                                                  label_list=list(test_label_table.ground_truth),
                                                  first_n_byte=args.first_n_byte,
                                                  padding_symbol=0,
                                                  transform=transform),
                                  batch_size=args.batch_size,
                                  shuffle=False,
                                  num_workers=args.use_cpu)

    ## load model
    ## model1: define as Raw byte based model, globally effective
    ## model2: define as Image based model, globally effective
