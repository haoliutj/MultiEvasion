...
config_path = '../../config/config_fire.yaml'
seed = 100
conf = yaml.load(open(config_path, 'r'))
...
tr_label_table.index=tr_label_table.index.str.upper() # upper the string
...
# Merge Tables and remove duplicate inputs based on inputname
...
# 1 means all, not 1 means random sample number of 'sample_cnt'
if sample_cnt != 1:
...
cnn_model = FireEyeCNN(input_length=first_n_byte, window_size=window_size)
bce_loss = nn.BCEWithLogitsLoss()
adam_optim = optim.Adam([{'params':cnn_model.parameters()}], lr=learning_rate)
sigmoid = nn.Sigmoid()
...
while total_step < max_step:
    ...
    if valid_best_acc < np.mean(history['val_acc']):
        ...
        torch.save(cnn_model, chkpt_acc_path)
